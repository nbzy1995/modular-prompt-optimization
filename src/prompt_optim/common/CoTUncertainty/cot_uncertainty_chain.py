...
_SYS = (
    "Think step by step. Answer only if you are certain. "
    "If uncertain at the end, reply exactly: I don't know. "
    "If certain, provide one line starting with 'Final answer:' "
...
def _postprocess(text: str) -> str:
    t = (text or "").strip()
    low = t.lower().strip()
    if low in {"i don't know", "i dont know"}:
        return "I don't know"
    marker = "final answer:"
...
class CoTUncertaintyChain:
    def __init__(self, llm, task, questions, model_id: str):
        ...
        self.out_path = f"result/{self.model_id}_{self.task}_cot_uncertainty_results.json"
